...
import { TTLCache, generateCacheKey } from './cache';
  ...
  async fetch(options: FetchOptions = {}): Promise<FetchResult> {
    ...
    const content = await this.rateLimiter.execute(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `CelesTrak fetch failed: ${response.status} ${response.statusText}`
          );
        }
...
export class SpaceTrackSource {
  ...
  private authUrl = 'https://www.space-track.org/ajaxauth/login';
  private queryUrl = 'https://www.space-track.org/basicspacedata/query';
  private rateLimiter: RateLimiter;
  private cache: TTLCache<string>;
  private config: DataSourceConfig;
  private sessionCookie: string | null = null;
  private lastAuth: number = 0;
  private authTTL = 7200000; // 2 hours

  constructor(config: Partial<DataSourceConfig> = {}) {
    if (!config.credentials?.username || !config.credentials?.password) {
      throw new Error(
        'Space-Track.org requires username and password credentials'
      );
    }
  ...
  private async authenticate(): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(
        `Space-Track authentication failed: ${response.status} ${response.statusText}`
      );
    }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            if (!retryResponse.ok) {
              throw new Error(
                `Space-Track fetch failed: ${retryResponse.status} ${retryResponse.statusText}`
              );
            }
          ...
          throw new Error(
            `Space-Track fetch failed: ${response.status} ${response.statusText}`
          );
        }
   ...
   */
  private jsonToTLE(json: any[]): string {
    ...
    return json
      .map(item => {
        ...
        // Add TLE lines
        if (item.TLE_LINE1) lines.push(item.TLE_LINE1);
        if (item.TLE_LINE2) lines.push(item.TLE_LINE2);
 ...
 * Public API, no authentication required
 */
export class AMSATSource {
  private baseUrl = 'https://www.amsat.org/tle/current/nasabare.txt';
  private rateLimiter: RateLimiter;
  private cache: TTLCache<string>;
  private config: DataSourceConfig;
  ...
  async fetch(options: FetchOptions = {}): Promise<FetchResult> {
    ...
    const content = await this.rateLimiter.execute(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `AMSAT fetch failed: ${response.status} ${response.statusText}`
          );
        }
  ...
  async fetch(options: FetchOptions = {}): Promise<FetchResult> {
    ...
    const content = await this.rateLimiter.execute(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Custom source fetch failed: ${response.status} ${response.statusText}`
          );
        }
...
export class DataSourceManager {
  private sources = new Map<
    string,
    CelesTrakSource | SpaceTrackSource | AMSATSource | CustomSource
  >();
  ...
  register(
    name: string,
    source: CelesTrakSource | SpaceTrackSource | AMSATSource | CustomSource,
    options: { primary?: boolean; failover?: boolean } = {}
  ): void {
  ...
  getSource(
    name: string
  ): CelesTrakSource | SpaceTrackSource | AMSATSource | CustomSource | undefined {
...
export function validateFreshness(
  tle: ParsedTLE,
  maxAgeMs: number = 259200000 // 3 days default
): FreshnessValidation {

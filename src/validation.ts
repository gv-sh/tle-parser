 ...
 */
export const ORBITAL_PARAMETER_RANGES = {
 ...
 */
export const SATELLITE_NUMBER_RANGES = {
  min: 1,
  max: 999999,
  // Historical cutoffs
 ...
 */
export const EPOCH_CONSTRAINTS = {
 ...
 */
export const DESIGNATOR_CONSTRAINTS = {
  yearMin: 57, // 1957
 ...
 */
export function convertEpochToDate(epochYear: number, epochDay: number): EpochDate {
 ...
 */
export function validateEpochDate(
  epochYear: number,
  epochDay: number
  ...
  if (epochYear < EPOCH_CONSTRAINTS.minYear || epochYear > EPOCH_CONSTRAINTS.maxYear) {
    errors.push(
      `Epoch year ${epochYear} outside valid range ` +
  ...
  if (epochDay < EPOCH_CONSTRAINTS.minDayOfYear || epochDay > EPOCH_CONSTRAINTS.maxDayOfYear) {
    errors.push(
      `Epoch day ${epochDay} outside valid range ` +
  ...
  if (Math.floor(epochDay) > maxDayForYear) {
    errors.push(
      `Day ${Math.floor(epochDay)} exceeds maximum for ${fullYear} ` +
 ...
 */
export function calculateEpochAge(
  epochYear: number,
  epochDay: number,
 ...
 */
export function validateEpochAge(
  epochYear: number,
  epochDay: number,
  options: { referenceDate?: Date; allowFuture?: boolean; maxAge?: number } = {}
): ValidationRuleResult {
  ...
  if (age < 0) {
    ...
    if (!options.allowFuture) {
      ...
      warnings.push(
        `Epoch is ${futureDays.toFixed(2)} days in the future ` +
  ...
  if (age > 0) {
    ...
    if (age > maxAge) {
      warnings.push(
        `Epoch is ${age.toFixed(2)} days old (threshold: ${maxAge} days)`
      );
    }
  }
 ...
 */
export function validateOrbitalParameter(
  paramName: keyof typeof ORBITAL_PARAMETER_RANGES,
  value: number,
  checkTypical: boolean = false
): ValidationRuleResult {
 ...
 */
export function validateAllOrbitalParameters(
  tle: ParsedTLE,
  checkTypical: boolean = false
): ValidationRuleResult[] {
  ...
  const parameterMap: Record<string, {
    field: keyof ParsedTLE;
    param: keyof typeof ORBITAL_PARAMETER_RANGES;
  }> = {
  ...
  for (const [key, mapping] of Object.entries(parameterMap)) {
    const value = parseFloat(tle[mapping.field] as string);
 ...
 */
export function validateChecksum(line: string, lineNumber: number): ValidationRuleResult {
  if (line.length < 69) {
    return {
      valid: false,
 ...
 */
export function normalizeAssumedDecimalNotation(value: string): number {
  value = value.trim();
...
export function normalizeScientificNotation(
  value: string,
  fieldType: 'assumedDecimal' | 'standard' | 'integer' = 'standard'
): number {
 ...
 * @returns Validation result
 */
export function validateSatelliteNumber(satelliteNumber: string | number): ValidationRuleResult {
 ...
 */
export function validateInternationalDesignator(designator: string): ValidationRuleResult {
 ...
 */
export function detectAnomalies(tle: ParsedTLE): AnomalyDetectionResult[] {
...
export function calculateQualityScore(
  ...
  components.anomalyScore = Math.max(
    0,
    QUALITY_SCORE_WEIGHTS.noAnomalies - anomalyPenalty * 2
  );
  ...
  const designatorResult = validateInternationalDesignator(
    tle.internationalDesignatorYear + tle.internationalDesignatorLaunchNumber + tle.internationalDesignatorPiece
  );
 ...
 */
export function sanitizeField(value: string, fieldName: string): FieldSanitizationResult {
  let sanitized = value;
 ...
 */
export function sanitizeAllFields(tle: ParsedTLE): Map<string, FieldSanitizationResult> {
  const results = new Map<string, FieldSanitizationResult>();
...
export const DEFAULT_VALIDATION_RULES: ValidationRule[] = [
  ...
  },
  {
    ...
    severity: 'error',
    enabled: true,
    validate: (value: string) => validateSatelliteNumber(value)
  },
  {
    name: 'epochDate',
  ...
  },
  {
    ...
    severity: 'warning',
    enabled: true,
    validate: (tle: ParsedTLE, context: { referenceDate?: Date }) => {
      const year = parseInt(tle.epochYear, 10);
      const day = parseFloat(tle.epoch);
...
export function createValidationRule<T = any>(
  name: string,
  description: string,
  validateFn: (value: T, context?: any) => ValidationRuleResult,
  options: {
    severity?: ErrorSeverity;
    enabled?: boolean;
    errorMessage?: string;
...
export class ValidationRuleManager {
  ...
  constructor(initialRules: ValidationRule[] = DEFAULT_VALIDATION_RULES) {
...
export function generateValidationReport(
  ...
  // Range validation
  const rangeResults: { valid: ValidationRuleResult[]; invalid: ValidationRuleResult[] } = {
    valid: [],
    invalid: []
  };

...
export interface TLEComparison {
  ...
  readonly timeDifference: number; // in days
...
export interface OrbitalDecayAnalysis {
  ...
  readonly decayRate: number; // change in mean motion per day
...
export interface TLEUpdateFrequencyStats {
  ...
  readonly averageUpdateInterval: number; // in days
...
export interface TLEQualityMetrics {
  ...
  readonly freshness: number; // 0-100
...
export interface ConjunctionProbability {
  ...
  readonly probabilityOfCollision: number; // 0-1
  readonly minimumDistance: number; // in kilometers
...
export interface RadarValidationResult {
  ...
  readonly positionError: number; // in kilometers
  readonly velocityError: number; // in km/s
  readonly confidence: number; // 0-1
 ...
 */
export function compareTLEs(oldTLE: ParsedTLE, newTLE: ParsedTLE): TLEComparison {
  const differences: TLEFieldDiff[] = [];
  ...
  for (const field of fieldsToCompare) {
    const oldValue = parseFloat(oldTLE[field as keyof ParsedTLE] as string);
    const newValue = parseFloat(newTLE[field as keyof ParsedTLE] as string);
  ...
  const significantChanges = differences.filter(diff => {
    if (diff.field === 'meanMotion' && Math.abs(diff.absoluteChange || 0) > 0.001) return true;
    if (diff.field === 'inclination' && Math.abs(diff.absoluteChange || 0) > 0.1) return true;
    if (diff.field === 'eccentricity' && Math.abs(diff.absoluteChange || 0) > 0.0001) return true;
    if (diff.percentChange && Math.abs(diff.percentChange) > 1) return true;
    return false;
  });
 ...
 */
export function generateTLEDiff(oldTLE: ParsedTLE, newTLE: ParsedTLE): string {
 ...
 */
export function assessTLEStaleness(tle: ParsedTLE, referenceDate: Date = new Date()): TLEStaleness {
  ...
  let staleness: TLEStaleness['staleness'];
  let isStale: boolean;
 ...
 */
export function analyzeOrbitalDecay(tles: readonly ParsedTLE[]): OrbitalDecayAnalysis {
  if (tles.length < 2) {
    return {
      isDecaying: false,
      decayRate: 0,
  ...
  let severity: OrbitalDecayAnalysis['severity'] = 'none';
  let estimatedLifetimeDays: number | null = null;
 ...
 */
export function calculateUpdateFrequency(tles: readonly ParsedTLE[]): TLEUpdateFrequencyStats {
  if (tles.length < 2) {
    return {
  ...
  let updatePattern: TLEUpdateFrequencyStats['updatePattern'];
 ...
 */
export function detectAnomalies(currentTLE: ParsedTLE, previousTLE?: ParsedTLE): AnomalyDetection {
  if (!previousTLE) {
    return {
      hasAnomaly: false,
      anomalyType: 'none',
 ...
 */
export function analyzeConstellation(tles: readonly ParsedTLE[], constellationName: string): ConstellationAnalysis {
  const orbitTypes: Record<OrbitType, number> = {
    LEO: 0,
    MEO: 0,
    GEO: 0,
    HEO: 0,
 ...
 */
export function calculateQualityMetrics(tle: ParsedTLE, referenceDate: Date = new Date()): TLEQualityMetrics {
  const issues: string[] = [];
  ...
  for (const field of requiredFields) {
    if (!tle[field as keyof ParsedTLE] || tle[field as keyof ParsedTLE] === '') {
  ...
  switch (staleness.staleness) {
    case 'fresh': freshness = 100; break;
    case 'recent': freshness = 85; break;
    case 'old': freshness = 60; break;
    case 'very_old': freshness = 30; break;
    case 'ancient': freshness = 0; break;
  }
  ...
  // Assign grade
  let grade: TLEQualityMetrics['grade'];
 ...
 */
export function analyzeTrend(tles: readonly ParsedTLE[], parameter: keyof ParsedTLE): TLETrendAnalysis {
  if (tles.length < 3) {
    return {
      satelliteNumber: tles[0]?.satelliteNumber1 || 'unknown',
  ...
  for (const tle of sortedTLEs) {
    ...
    if (fieldValue) {
      const value = parseFloat(fieldValue as string);
      if (!isNaN(value)) {
        values.push(value);
        const epoch = parseEpoch(tle.epochYear, tle.epoch);
  ...
  // Determine trend
  let trend: TLETrendAnalysis['trend'];
 ...
 */
export function classifyOrbitType(tle: ParsedTLE): OrbitType {
  const meanMotion = parseFloat(tle.meanMotion);
  const eccentricity = parseFloat(tle.eccentricity);
  const inclination = parseFloat(tle.inclination);
 ...
 */
export function calculateConjunctionProbability(
  tle1: ParsedTLE,
  tle2: ParsedTLE,
  ...
  let probability = 0;
  let riskLevel: ConjunctionProbability['riskLevel'] = 'low';
  const recommendations: string[] = [];
 ...
 */
export function validateAgainstRadar(
  _tle: ParsedTLE,
  _radarPosition: { latitude: number; longitude: number; altitude: number },
  _radarObservation: { range: number; azimuth: number; elevation: number; time: Date }
 ...
 */
export function groupIntoOrbitalFamilies(
  tles: readonly ParsedTLE[],
  tolerances = {
  ...
  for (let i = 0; i < tles.length; i++) {
    const tle1 = tles[i]!;
    if (assigned.has(tle1.satelliteNumber1)) continue;

    const familyMembers: string[] = [tle1.satelliteNumber1];
    ...
    for (let j = i + 1; j < tles.length; j++) {
      const tle2 = tles[j]!;
      if (assigned.has(tle2.satelliteNumber1)) continue;
      ...
      if (
        Math.abs(inc1 - inc2) <= tolerances.inclinationTolerance &&
    ...
    let purpose = 'Unknown';
    if (inc1 > 95 && inc1 < 100) purpose = 'Sun-synchronous Earth observation';
    else if (inc1 < 10 && alt1 > 35000) purpose = 'Geostationary communications';
    else if (inc1 > 50 && inc1 < 60 && alt1 > 19000 && alt1 < 24000) purpose = 'Navigation (GPS/GLONASS)';
    else if (alt1 < 600) purpose = 'Low Earth orbit constellation';
 ...
 */
function calculateAltitudeFromMeanMotion(meanMotion: number): number {
...
function generateComparisonSummary(
  differences: readonly TLEFieldDiff[],
  significantChanges: readonly TLEFieldDiff[],
 ...
 */
function estimateOrbitalPlanes(raanValues: readonly number[]): number {
  if (raanValues.length < 2) return 1;
  ...
  const planes: number[][] = [];
  let currentPlane: number[] = [sorted[0]!];

  ...
  try {
    ...
    while (i < lines.length) {
      ...
      if (currentLine && nextLine && nextNextLine && !currentLine.startsWith('1 ')) {
        ...
        try {
          const parsed = parseTLE(tleLine, options);
          tles.push(parsed);
          i += 3;
        } catch (error) {
          errors.push({
 ...
 */
export async function parseFromFileList(
  fileList: FileList,
  options?: TLEParseOptions
): Promise<FileParseResult[]> {
 ...
 * @returns Array of parse results
 */
export async function parseFromDragEvent(
  event: DragEvent,
  options?: TLEParseOptions
): Promise<FileParseResult[]> {
 ...
 * @returns Blob URL
 */
export function createDownloadLink(tles: ParsedTLE[]): string {
 ...
 */
export function downloadTLEFile(tles: ParsedTLE[], fileName: string = 'tles.txt'): void {

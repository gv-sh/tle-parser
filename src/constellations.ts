 ...
 */
export function listConstellations(): string[] {
 ...
 */
export function createConstellationFilter(
  ...
  if (constellation.catalogNumbers && constellation.catalogNumbers.length > 0) {
    filter.satelliteNumber = (satNum: string) => {
  ...
  if (constellation.namePatterns && constellation.namePatterns.length > 0) {
    ...
    filter.satelliteName = (name: string) => {
      const matchesPattern = constellation.namePatterns!.some(pattern =>
        pattern.test(name)
      );
 ...
 */
export function matchesConstellation(
  tle: ParsedTLE,
  constellationName: string
): boolean {
  const constellation = getConstellation(constellationName);
  ...
  if (constellation.namePatterns && tle.satelliteName) {
    const matchesName = constellation.namePatterns.some(pattern =>
      pattern.test(tle.satelliteName!)
    );
...
export function groupByConstellation(
  tles: ParsedTLE[]
): Map<string, ParsedTLE[]> {
  const groups = new Map<string, ParsedTLE[]>();

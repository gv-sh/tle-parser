...
exports.TLEStateMachineParser = exports.RecoveryAction = exports.ErrorSeverityEnum = exports.ParserState = void 0;
...
})(ErrorSeverityEnum || (exports.ErrorSeverityEnum = ErrorSeverityEnum = {}));
/**
...
class TLEStateMachineParser {
    ...
    parse(tleString) {
        ...
        if (typeof tleString !== 'string') {
            this.addIssue(ErrorSeverityEnum.CRITICAL, errorCodes_1.ERROR_CODES.INVALID_INPUT_TYPE, 'TLE data must be a string', { inputType: typeof tleString });
            this.transition(ParserState.ERROR);
            return this.getResult();
        }
        if (tleString.length === 0) {
            this.addIssue(ErrorSeverityEnum.CRITICAL, errorCodes_1.ERROR_CODES.EMPTY_INPUT, 'TLE string cannot be empty', { inputLength: 0 });
            this.transition(ParserState.ERROR);
            return this.getResult();
        }
        // Start parsing
    ...
    detectFormat(tleString) {
        ...
        // Validate line count
        if (lines.length < 2) {
            this.addIssue(ErrorSeverityEnum.CRITICAL, errorCodes_1.ERROR_CODES.INVALID_LINE_COUNT, `TLE must contain at least 2 lines (found ${lines.length})`, { expected: '2 or 3', actual: lines.length });
        ...
        if (lines.length > 3) {
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_LINE_COUNT, `TLE should contain 2 or 3 lines (found ${lines.length})`, { expected: '2 or 3', actual: lines.length });
            if (this.options.attemptRecovery) {
            ...
            if (lines.length === 3) {
                ...
                if (firstChar === '1' || firstChar === '2') {
                    this.addIssue(ErrorSeverityEnum.WARNING, errorCodes_1.ERROR_CODES.SATELLITE_NAME_FORMAT_WARNING, 'First line starts with "1" or "2", might be missing satellite name', { firstLine: lines[0] });
                }
                this.context.hasName = true;
    ...
    parseSatelliteName() {
        ...
        if (nameLine.length > 24) {
            this.addIssue(ErrorSeverityEnum.WARNING, errorCodes_1.ERROR_CODES.SATELLITE_NAME_TOO_LONG, 'Satellite name exceeds recommended 24 characters', { length: nameLine.length, name: nameLine });
        }
        if (nameLine[0] === '1' || nameLine[0] === '2') {
            this.addIssue(ErrorSeverityEnum.WARNING, errorCodes_1.ERROR_CODES.SATELLITE_NAME_FORMAT_WARNING, 'Satellite name starts with "1" or "2", might be incorrectly formatted', { name: nameLine });
        }
    }
    /**
     * Parse TLE Line 1
    ...
    parseLine1() {
        ...
        // Check line length
        if (line1.length !== 69) {
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_LINE_LENGTH, `Line 1 must be exactly 69 characters (got ${line1.length})`, { line: 1, expected: 69, actual: line1.length });
            if (this.options.attemptRecovery) {
        ...
        if (this.parsedData.lineNumber1 !== '1') {
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_LINE_NUMBER, `Line 1 must start with '1' (got '${this.parsedData.lineNumber1}')`, { line: 1, expected: '1', actual: this.parsedData.lineNumber1 });
        }
        ...
        if (this.parsedData.classification &&
            ...
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_CLASSIFICATION, `Classification must be U, C, or S (got '${this.parsedData.classification}')`, { expected: validClassifications, actual: this.parsedData.classification });
        }
        // Validate checksum
        if (line1.length === 69) {
            ...
            if (!isNaN(actualChecksum) && checksum !== actualChecksum) {
                this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.CHECKSUM_MISMATCH, `Line 1 checksum mismatch (expected ${checksum}, got ${actualChecksum})`, { line: 1, expected: checksum, actual: actualChecksum });
                if (this.options.attemptRecovery) {
    ...
    parseLine2() {
        ...
        // Check line length
        if (line2.length !== 69) {
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_LINE_LENGTH, `Line 2 must be exactly 69 characters (got ${line2.length})`, { line: 2, expected: 69, actual: line2.length });
            if (this.options.attemptRecovery) {
        ...
        if (this.parsedData.lineNumber2 !== '2') {
            this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_LINE_NUMBER, `Line 2 must start with '2' (got '${this.parsedData.lineNumber2}')`, { line: 2, expected: '2', actual: this.parsedData.lineNumber2 });
        }
        // Validate checksum
        if (line2.length === 69) {
            ...
            if (!isNaN(actualChecksum) && checksum !== actualChecksum) {
                this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.CHECKSUM_MISMATCH, `Line 2 checksum mismatch (expected ${checksum}, got ${actualChecksum})`, { line: 2, expected: checksum, actual: actualChecksum });
                if (this.options.attemptRecovery) {
    ...
    validateCrossFields() {
        ...
        if (this.parsedData.satelliteNumber1 && this.parsedData.satelliteNumber2) {
            ...
            if (sat1 !== sat2) {
                this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.SATELLITE_NUMBER_MISMATCH, `Satellite numbers must match (Line 1: ${sat1}, Line 2: ${sat2})`, { line1Value: sat1, line2Value: sat2 });
            }
        }
        ...
        if (this.parsedData.eccentricity) {
            ...
            if (!isNaN(ecc) && (ecc < 0 || ecc > 1)) {
                this.addIssue(ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.VALUE_OUT_OF_RANGE, `Eccentricity must be between 0 and 1 (got ${ecc})`, { field: 'eccentricity', value: ecc, min: 0, max: 1 });
            }
        }
    ...
    validateNumericField(fieldName, min, max, displayName, warningOnly = false) {
        ...
        if (isNaN(value)) {
            this.addIssue(warningOnly ? ErrorSeverityEnum.WARNING : ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.INVALID_NUMBER_FORMAT, `${displayName} must be numeric (got '${fieldValue}')`, { field: fieldName, value: fieldValue });
            return;
        }
        if (value < min || value > max) {
            this.addIssue(warningOnly ? ErrorSeverityEnum.WARNING : ErrorSeverityEnum.ERROR, errorCodes_1.ERROR_CODES.VALUE_OUT_OF_RANGE, `${displayName} must be between ${min} and ${max} (got ${value})`, { field: fieldName, value, min, max });
        }
    }
    /**
...
//# sourceMappingURL=stateMachineParser.js.map